...
@app.route('/adminscan', methods=['GET', 'POST'])
def index():
    if request.method == 'POST':
        ...
        result = scan_website(site, pages)
...
def scan_website(site, pages):
    ...
    results.append(f"\t [+] Scanning {site}...\n\n")
    for admin in pages:
        admin = admin.replace("\n", "")
        admin = "/" + admin
        host = site + admin
        results.append(f"\t [#] Checking {host}...")
        ...
        if response.status == 200:
            var1 += 1
            found_pages.append(host)
            results.append(f"{host} Admin page found!")
        elif response.status == 404:
            pass
        elif response.status == 302:
            results.append(f"{host} Possible admin page (302 - Redirect)")
        else:
            results.append(f"{host} Interesting response: {response.status}")  

    connection.close()

    results.append("\n\nCompleted \n")
    results.append(f"{var1} Admin pages found")
    results.append(f"{var2} total pages scanned")
